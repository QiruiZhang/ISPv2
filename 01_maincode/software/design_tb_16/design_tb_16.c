...
int main() {
	...
	int mode 		= 0;//1: yuv 2:h264 3:full
	...
	if(mode==1){
	    ...
	    while(1){  
			...
			if(frame_flag){
				...
				for (row_index = 0; row_index < mcb_per_col; row_index=row_index+1) 	{
					for (col_index = 0; col_index < mcb_per_row; col_index=col_index+1)	{
						
						arb_debug_reg(0xDB, addr_index);
						for(i=0; i<(64); i=i+1){
							flsif_senddata(*((uint32_t *)(p_MCBRD_SRAM_START+addr_index)));
							while(1) {
								if (flash_flag) {
									flash_flag =0;
									delay(1);
			...
			if(frame_flag){
				...
				for (row_index = 0; row_index < mcb_per_col; row_index=row_index+1) 	{
					for (col_index = 0; col_index < mcb_per_row; col_index=col_index+1)	{
						...
						arb_debug_reg(0xDB, addr_index);
						for(i=0; i<(32); i=i+1){
							flsif_senddata(*((uint32_t *)(p_MCBRD_SRAM_START+1200+addr_index)));
							while(1) {
								if (flash_flag) {
									flash_flag =0;
									delay(1);
	    ...
	    while(1){  
			...
			if(frame_flag){
				...
				for(mcb_row=0; mcb_row<mcb_per_col; mcb_row = mcb_row + 1){
