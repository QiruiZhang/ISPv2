...
int main() {
	...
	imgif_ref_cd_mode_enable(3);
    	//p_FRMMEM_SSADDR->frmmem_ssaddr_y  = 4096;
    	//p_FRMMEM_SSADDR->frmmem_ssaddr_uv = 2560;
	imgif_initialize();
	...
	//	flsif_senddata(*((uint32_t *)(p_MINMAX_SRAM_START+i)));
	//	while(1) {
	//		if (flash_flag) {
	//			flash_flag =0;
	//			delay(1);
	...
	//	delay(1);
	//}
//	p_IMGIF_IMG_INFO->as_int = 0x00; //direct_mem_access_on
//	for(i=0; i<(1200*2); i=i+1){
//		flsif_senddata(*((uint32_t *)(p_COMP_SRAM_START+i)));
//		while(1) {
//			if (flash_flag) {
//				flash_flag =0;
//				delay(1);
...
//		delay(1);
//	}
//	p_IMGIF_IMG_INFO->as_int = 0x10; //direct_mem_access_off
//
	//mode=1, yuv only flsout, mode=2, h264 flsout
	int mode =4;
	if (mode == 1){
	   while(1){  
	       	delay(1);
	   ...
	   while(1){  
	       ...
	       //		for(mcb_index=1199; mcb_index>=1190; mcb_index=mcb_index-1)	{
	   	    	for(mcb_index=0; mcb_index<1200; mcb_index=mcb_index+1)	{
	       			arb_debug_reg(0xDB, mcb_index);
	       			for(i=0; i<(32); i=i+1){
			...
			if(frame_flag){
				...
				for(mcb_row=0; mcb_row<30; mcb_row = mcb_row + 1){
					for(mcb_col=0; mcb_col<40; mcb_col = mcb_col + 1){
						...
						while(1) { delay(100); if (flash_flag) { flash_flag =0; break; } }
						delay(1);
					}
				}
				delay(1);
		...
		int btstrm_len=0;
		int h264_rdata;
		h264_config(p_H264_COMPMEM_SADDR_YREF->as_int, p_H264_COMPMEM_SADDR_UVREF->as_int, 10,0);//quality factor = 10
		flsif_initialize(0x3, 0x3); //AHB path
		i=0;
    		while(1){  
			delay(1); if(frame_flag){ delay(1); break; }
		}
	...
	delay(10000);
	   ...
	   while(1){  
	       	delay(1);
		if(mbusrx_flag){ break; }
	   }
	   for(i=0; i<4096; i=i+1)	{
	   	flsif_senddata(*((uint32_t *)(p_CD_SRAM_START+i)));
	  	while(1) { if (flash_flag) { flash_flag =0; delay(1); break; } delay(1); }
	   }
	}
	else {
	   while(1){  
	       	...
	       	if(frame_flag){
	       		...
	       		for(mcb_index=0; mcb_index<1200; mcb_index=mcb_index+1)	{
	       			arb_debug_reg(0xDB, mcb_index);
	       			for(i=0; i<(64); i=i+1){
	       				flsif_senddata(*((uint32_t *)(p_MCBRD_SRAM_START+mcb_index+i)));
	       				while(1) {
	       					if (flash_flag) {
	       						flash_flag =0;
	       						delay(1);
	   ...
	   while(1){  
	       	...
	       	if(frame_flag){
	       		...
	       		for(mcb_index=0; mcb_index<1200; mcb_index=mcb_index+1)	{
	       			arb_debug_reg(0xDB, mcb_index);
	       			for(i=0; i<(32); i=i+1){
	       				flsif_senddata(*((uint32_t *)(p_MCBRD_SRAM_START+1200+mcb_index+i)));
	       				while(1) {
	       					if (flash_flag) {
	       						flash_flag =0;
	       						delay(1);
