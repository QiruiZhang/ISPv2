...
function [Seq_r_Luma,Seq_r_ChB,Seq_r_ChR,idx, dbg_buf] = Dec_Frame2_bu(idx,bitstream, ref_frame, CD_map)
global bw_pixel;
%global QP;
[h,w,~] = size(ref_frame);
h
...
while(1)
    ...
    if (CD_map(floor(i/16)+1,floor(j/16)+1) == 1)
            ...
            for m = i:4:i+15
                for n = j:4:j+15
                    ...
                    if( err_flag == 1 )
                        mcb_ss_idx(1) = mcb_ss_idx(1) + 1;
                        idx = mcb_ss_idx(1);
                        mode_prev_Luma = mode_prev_Luma_lastmcb(1);
                        errcnt(1) = errcnt(1) + 1;
                        fprintf("Error("+errcnt(1)+"): mode="+num2str(mode)+", m="+num2str(m)+", n="+num2str(n)+"\n");
                        break;
                    end
                    mode_prev_Luma = mode;
                end
        ...
        if(err_flag ~= 1)
            ...
            modeB = mode_prev_ChromaB(1) + mode_diff;
            ...
            modeR = mode_prev_ChromaR(1) + mode_diff;
            ...
            if( err_flagC == 1)
                ...
                mode_prev_Luma = mode_prev_Luma_lastmcb(1);
                errcnt(1) = errcnt(1) + 1;
                fprintf("Chorma Error("+errcnt(1)+"): modeB="+num2str(modeB)+"modeR="+num2str(modeR)+", m="+num2str(m)+", n="+num2str(n)+"\n");
            else
                %             mode_pprev_ChromaB = mode_prev_ChromaB;
                %             mode_prev_ChromaB = mode;
                mode_prev_ChromaB = update_stack(mode_prev_ChromaB, modeB, 1, num_stack);
                %             mode_pprev_ChromaR = mode_prev_ChromaR;
                %             mode_prev_ChromaR = mode;
                mode_prev_ChromaR = update_stack(mode_prev_ChromaR, modeR, 1, num_stack);
                
                %%%stack%%%
                %             % 1 MCB before
                %             mode_pprev_Luma_lastmcb = mode_prev_Luma_lastmcb;
                %             mcb_pprev_ss_idx = mcb_ss_idx;
                %             % 0.x MCB before
                %             mode_prev_Luma_lastmcb = mode_prev_Luma;
                %             mcb_ss_idx = idx;
                mode_prev_Luma_lastmcb = update_stack(mode_prev_Luma_lastmcb, mode_prev_Luma, 1, num_stack);
                mcb_ss_idx = update_stack(mcb_ss_idx, idx, 1, num_stack);

		%%
                errcnt = update_stack(errcnt, 0, 1, num_stack); %start from 0 err
            end
    ...
    if(err_flag ~= 1 && err_flagC ~= 1) % no error case
        mcb_idx = mcb_idx+1;
    elseif (err_flag ~= 1 && err_flagC == 1) %error in chorma
        mcb_idx = mcb_idx;
    else % errors in Luma
        err_flag = 0;
        for i=1:1:num_stack
            mcb_idx = mcb_idx - 1;
            mode_prev_ChromaR = update_stack(mode_prev_ChromaR, 0, 0, num_stack);
            mode_prev_ChromaB = update_stack(mode_prev_ChromaB, 0, 0, num_stack);
            mode_prev_Luma_lastmcb = update_stack(mode_prev_Luma_lastmcb, 0, 0, num_stack);
            mode_prev_Luma = mode_prev_Luma_lastmcb(1);
            mcb_ss_idx = update_stack(mcb_ss_idx, 0, 0, num_stack);
            errcnt = update_stack(errcnt, 0, 0, num_stack);
            if(errcnt(1) < 2)
                break;
            end
        end
        %% 0.x MCB before
        %%         mode_prev_ChromaR = mode_pprev_ChromaR;
        %%         mode_prev_ChromaB = mode_pprev_ChromaB;
        %mode_prev_ChromaR = update_stack(mode_prev_ChromaR, 0, 0, num_stack);
        %mode_prev_ChromaB = update_stack(mode_prev_ChromaB, 0, 0, num_stack);
        %%         mode_prev_Luma = mode_pprev_Luma_lastmcb;
        %mode_prev_Luma_lastmcb = update_stack(mode_prev_Luma_lastmcb, 0, 0, num_stack);
        %mode_prev_Luma = mode_prev_Luma_lastmcb(1);
       	%mcb_ss_idx(1) = mcb_ss_idx(1) + 1;
        mcb_ss_idx(1) = mcb_ss_idx(1) + 1;
