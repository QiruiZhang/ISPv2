...
function [Seq_r_Luma,Seq_r_ChB,Seq_r_ChR,idx, bitstream] = DEC_Frame_FIFOdbg(idx,bitstream, ref_frame, CD_map)
global bw_pixel;
%global QP;
[h,w,~] = size(ref_frame);
%--------------------------------------------------
...
for i = 1:bs:h
    for j = 1:bs:w
        if (CD_map(floor(i/16)+1,floor(j/16)+1) == 1)
            ...
            fprintf("idx after this McB is " + num2str(idx) + "\n");
            idx_divby_32 = ceil((idx-1)/32);
            idx = idx_divby_32*32 + 1;
            % bitstream = [bitstream(1:(idx-1)), bitstream((idx_divby_32*32+1):end)];
        end
    end
end
