...
function [Seq_r_Luma,Seq_r_ChB,Seq_r_ChR,idx] = DEC_Frame_debug(idx,bitstream, ref_frame, CD_map)
global bw_pixel;
%global QP;
[h,w,~] = size(ref_frame);
%--------------------------------------------------
        ...
        if (CD_map(floor(i/16)+1,floor(j/16)+1) == 1)
            ...
            for m = i:4:i+15
                for n = j:4:j+15
                    ...
                    [Seq_r_Luma(m:m+3,n:n+3),idx,error_flag] = dec_mb_4(idx,bitstream,mode,Seq_r_Luma,m,n);
                    if (error_flag)
                        break;
                    end
                    mode_prev_Luma = mode;
            ...
            if (error_flag)
                fprintf("mode error occurred!\n");
                idx = idx_mcbstart;
                mode_prev_Luma = mode_mcbstart;
                fprintf("idx after rolling back is: " + num2str(idx));
                bitstream = [bitstream(1:(idx-1)), bitstream((idx+1):end)];
                for m = i:4:i+15
                    for n = j:4:j+15
                        fprintf("idx before dec_golomb: " + num2str(idx) + "\n");
                        [mode_diff,idx]= dec_golomb(idx,bitstream,1);
...
%-----------------------------------------------------------------
function [Xi,k,err_flag] = dec_mb_4(k,bits,mode,Seq_r_Luma,i,j)
err_flag = false;
