function [bitstream,Rec_Frame_Luma,Rec_Frame_ChromaB,Rec_Frame_ChromaR] = ENC_Frame(cur_frame, ref_frame, CD_map)
...
%Convert to YUV
Iyuv_cur  = rgb2ycbcr(cur_frame); 
Iyuv_cur = double(Iyuv_cur);
Iy_cur = Iyuv_cur(:,:,1);
Iu_cur = Iyuv_cur(:,:,2);
Iv_cur = Iyuv_cur(:,:,3);

%subsample as 4:2:0 and extend to 12-bit pixel   
Y_cur = Iy_cur(:,:) * 2^(bw_pixel-8);
U_cur = Iu_cur(1:2:h,1:2:w) * 2^(bw_pixel-8);         
V_cur = Iv_cur(1:2:h,1:2:w) * 2^(bw_pixel-8);
